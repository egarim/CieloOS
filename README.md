...
**Sessions**
- **Desk profiles** — a user is created as an **Office**, **.NET developer** or **Marketing** desk. The profile decides the session image, the dock and what the home starts with: a developer desk carries a .NET SDK, Uno Platform templates and VS Code with the Uno extension, so `dotnet new unoapp` works on a fresh desk. A desk is two images — the desktop the person uses and the console their agent works in, so a .NET desk's agent can run `dotnet` too. Both are layered on the shared images and **built on demand**, so a machine only pays for the desks it actually uses.
